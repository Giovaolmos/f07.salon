...
import { getAppointmentsByHairdresser } from "../../helpers/hairdressers/getAppointmentsByHairdresser";
  ...
  const handleCancelAppointment = async (appointmentId: string) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        const hairdresserData = localStorage.getItem("hairdresserData");
        if (!hairdresserData)
          throw new Error("No se encontró información del peluquero");
        const { id } = JSON.parse(hairdresserData);
        const updatedAppointments = await getAppointmentsByHairdresser(id);
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const { id } = JSON.parse(hairdresserData);
        const data = await getAppointmentsByHairdresser(id);
      ...
      {appointments.length === 0 ? (
        <div className="text-center bg-yellow-800/60 rounded-lg p-8 max-w-md mx-auto">
          ...
          <p className="text-yellow-200 text-md">
            Cuando los clientes reserven contigo, las citas aparecerán aquí
          </p>
        </div>
      ) : (
        <div>
...
export default HomeHairdresser;
